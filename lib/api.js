...
const { merge, getPullRequest } = require("./merge");
...
async function handleCheckUpdate(context, eventName, event) {
  ...
  if (action !== 'completed') {
    ...
    if (payload.conclusion === 'success') {
      await updateAndMergePullRequest(context, payload.pull_requests[0], event.organization.owner.login);
    } else {
...
async function updateAndMergePullRequest(context, pr, login) {
    const { octokit } = context;

    let fullPR = await getPullRequest(octokit, pr);

    logger.info("Full PR information: ", fullPR);

    fullPR.base.repo.owner = { login: login };
