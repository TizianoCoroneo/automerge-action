...
async function handleCheckUpdate(context, eventName, event) {
  ...
  if (action !== 'completed') {
    ...
    if (payload.conclusion === 'success') {
      logger.info("Repo: ", event.repository);
      logger.info("Owner: ", event.repository.owner);
      logger.info("Login: ", event.repository.owner.login);

      await updateAndMergePullRequest(context, event.repository.owner.login, payload.pull_requests[0]);
    } else {
...
async function updateAndMergePullRequest(context, owner_login, pr) {
    const { octokit } = context;

    let fullPR = await getPullRequest(octokit, owner_login, pr);

    logger.info("Full PR information: ", fullPR);

    fullPR.base.owner = { login: owner_login };

    await update(context, fullPR);
    await merge(context, fullPR);
}
...
async function getPullRequest(octokit, owner_login, pullRequest) {
